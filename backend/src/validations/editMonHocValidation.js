...
const editMonHocValidation = async (req, res, next) => {
  try {
    const editDiemCondition = Joi.object({
      ...
      SOTIET_LT : Joi.number().empty().min(20).max(40).required().strict(),
      SOTIET_TH: Joi.number().empty().min(20).max(40).required().strict(),
...
export default editMonHocValidation